...
def transfer_test_set(raw_json_path="../evaluation_test_set/Evaluation_text.json", output_path="./CCKS_test.conll"):
    sample_text = load_json(raw_json_path)
    convert_test_to_conll(sample_text,output_path=output_path)
    print("将测试集json生成的conll保存路径为: "+output_path)

if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description="将测试集json转为conll格式")
    parser.add_argument('--raw_json_path', type=str, default="../evaluation_test_set/Evaluation_text.json", help="原始测试集json路径")
    parser.add_argument('--output_path', type=str, default="./CCKS_test.conll", help="输出conll文件路径")
    args = parser.parse_args()

    transfer_test_set(raw_json_path=args.raw_json_path, output_path=args.output_path)
